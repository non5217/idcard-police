...
function getBase64Img($file_path) {
    ...
    if (!$real_path || !file_exists($real_path)) return '';
...
function formatThaiDateShort($dateString) {
    ...
    if ($dateString == '9999-12-31') return "ตลอดชีพ";
...
<head>
    <meta charset="UTF-8">
    <title>Print Zebra - <?= htmlspecialchars($req['full_name']) ?></title>
    ...
    <style>
        ...
        #el_gov { font-size: var(--gov-font-size); }
        ...
        .set-row { display: grid; grid-template-columns: 2.5fr 1fr 1fr 1fr 1fr 1fr; gap: 4px; margin-bottom: 5px; align-items: center; }
        .set-row input { border: 1px solid #ccc; border-radius: 4px; padding: 2px 4px; font-size: 11px; width: 100%; text-align: center; }
        .set-row input:disabled { background-color: #f3f4f6; color: #9ca3af; cursor: not-allowed; }
        .set-head { font-size: 11px; font-weight: bold; color: #555; text-align: center; }
    </style>
</head>
<body class="flex flex-col h-screen overflow-y-auto">

    <div id="settingsSidebar" class="fixed top-0 left-0 h-screen w-[620px] bg-white shadow-[5px_0_20px_rgba(0,0,0,0.2)] z-50 transform -translate-x-full transition-transform duration-300 no-print flex">
        ...
        <div class="flex-1 h-full overflow-y-auto p-6 bg-white relative">
            <div class="flex justify-between items-center mb-4 border-b pb-4">
                ...
                <div class="flex gap-2">
                    <button id="lockBtn" onclick="toggleLock()" class="px-3 py-1 rounded text-sm font-bold shadow transition"></button>
                    <button onclick="resetDefaults()" class="bg-gray-400 hover:bg-gray-500 text-white px-3 py-1 rounded text-sm font-bold shadow" title="โหลดพิกัดเริ่มต้นใหม่"><i class="fas fa-undo"></i> รีเซ็ต</button>
                </div>
            </div>

            <div class="mb-4 bg-purple-50 p-3 rounded border border-purple-200">
                <label class="font-bold text-xs text-purple-900 mb-1 block"><i class="fas fa-save"></i> ฐานข้อมูล Pre-set (เลือกเพื่อโหลดค่า):</label>
                <div class="flex gap-2 items-center">
                    <select id="preset_select" class="border p-1 rounded w-full text-sm font-bold bg-white" onchange="loadPresetFromDB()">
                        <option value="default">-- ค่าเริ่มต้น (Default Config) --</option>
                    </select>
                    <button onclick="updateCurrentPreset()" class="bg-blue-600 text-white px-2 py-1 rounded text-xs font-bold hover:bg-blue-700 whitespace-nowrap"><i class="fas fa-save"></i> ทับ</button>
                    <button onclick="saveAsNewPreset()" class="bg-green-600 text-white px-2 py-1 rounded text-xs font-bold hover:bg-green-700 whitespace-nowrap"><i class="fas fa-plus"></i> ใหม่</button>
                    <button onclick="deleteCurrentPreset()" class="bg-red-500 text-white px-2 py-1 rounded text-xs font-bold hover:bg-red-600 whitespace-nowrap"><i class="fas fa-trash"></i></button>
                </div>
            </div>
            ...
            <div class="mb-4 bg-blue-50 p-3 rounded border border-blue-200 flex justify-between items-center gap-2">
                <div class="w-1/2">
                    ...
                    <div class="flex items-center gap-2">
                        <input type="number" id="inp_baseFontSize" step="0.5" class="border p-1 w-full text-center rounded font-bold" oninput="updateBaseSettings()"> 
                ...
                <div class="w-1/2">
                    ...
                    <div class="flex items-center gap-2">
                        <input type="number" id="inp_govFontSize" step="0.5" class="border p-1 w-full text-center rounded font-bold" oninput="updateBaseSettings()"> 
                        <span class="text-sm font-bold text-gray-600">pt</span>
                    </div>
                </div>
            </div>
            ...
            <div class="set-row border-b pb-1 mb-2 sticky top-0 bg-white pt-2">
                ...
                <div class="set-head">X</div><div class="set-head">Y</div><div class="set-head">W</div><div class="set-head text-green-700">H</div><div class="set-head text-blue-600">Pt</div>
            </div>
        ...
        <div class="absolute -right-11 top-1/3 transform -translate-y-1/2">
            <button onclick="toggleSidebar()" id="sidebarTabBtn" class="bg-blue-800 text-white p-2 rounded-r-lg shadow-[4px_0_8px_rgba(0,0,0,0.3)] flex flex-col items-center gap-3 border border-blue-900 border-l-0 transition-all hover:bg-blue-700">
    ...
    <div id="preview-wrapper" class="flex-1 flex flex-col items-center justify-start w-full relative pt-10">
        ...
        <div class="no-print mb-4 text-center">
            ...
            <div class="flex justify-center gap-4 mb-4">
                <button onclick="printAndComplete()" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-8 rounded-lg shadow-lg text-lg transition transform hover:scale-105 border-2 border-white">
                    <i class="fas fa-print"></i> สั่งพิมพ์และเปลี่ยนสถานะทันที
                </button>
            </div>
            
            <div class="flex items-center justify-center gap-4 bg-white px-6 py-3 rounded-full shadow border border-gray-200">
                <span class="text-sm font-bold text-gray-700"><i class="fas fa-search-plus text-blue-600"></i> ซูมดูพรีวิว:</span>
                <button onclick="setZoom(1)" class="px-3 py-1 bg-gray-100 hover:bg-gray-200 rounded-full text-xs font-bold border transition">100%</button>
                <input type="range" id="zoomSlider" min="0.5" max="4" step="0.1" value="1" class="w-48 cursor-pointer" oninput="updateZoom(this.value)">
                <span id="zoomValue" class="text-sm font-bold text-blue-700 w-12 text-right">100%</span>
            </div>
        </div>

        <div class="card-box" id="print-card-area">
            <img src="<?= getBase64Img($req['photo_path']) ?>" id="el_photo" style="position:absolute; object-fit:cover; object-position:top center; z-index: 1;">
            ...
            <div class="el flex items-center" id="el_no_val" style="z-index: 10;"><?= $card_serial_thai ?></div>
            <div class="el flex items-center" id="el_name_lbl" style="z-index: 10;">ชื่อ</div>
            <div class="el flex items-center" id="el_name_val" style="z-index: 10;"><span class="shrink-text"><?= htmlspecialchars($full_name_rank) ?></span></div>
            <div class="el flex items-center" id="el_pos_lbl" style="z-index: 10;">ตำแหน่ง</div>
            <div class="el flex items-center" id="el_pos_val" style="z-index: 10;"><span class="shrink-text"><?= htmlspecialchars($req['position']) ?></span></div>
            <div class="el flex items-center" id="el_org_val" style="z-index: 10;"><span class="shrink-text"><?= htmlspecialchars($req['org_name']) ?></span></div>
            <div class="el flex items-center" id="el_id_lbl" style="z-index: 10;">เลขประจำตัวประชาชน</div>
            <div class="el flex items-center" id="el_id_val" style="z-index: 10;"><?= $id_card_thai ?></div>
            
            <?php if($req['signature_file']): ?>
                <img src="<?= getBase64Img($req['signature_file']) ?>" id="el_app_sig" style="position:absolute; object-fit:contain; z-index: 1;">
            <?php endif; ?>

            <div class="el flex items-center" id="el_app_rank" style="z-index: 10;"><span class="shrink-text"><?= htmlspecialchars($req['rank_name']) ?></span></div>
            ...
            <div class="el flex items-center" id="el_blood_val" style="z-index: 10;"><?= $blood_th ?></div>
            <div class="el flex items-center" id="el_issue_lbl" style="z-index: 10;">วันออกบัตร</div>
            <div class="el flex items-center" id="el_issue_val" style="z-index: 10;"><?= $issue_date_thai ?></div>
            <div class="el flex items-center" id="el_exp_lbl" style="z-index: 10;">บัตรหมดอายุ</div>
            <div class="el flex items-center" id="el_exp_val" style="z-index: 10;"><?= $expiry_date_thai ?></div>
            
            <?php if($issuer_sig_path): ?>
                <img src="<?= getBase64Img($issuer_sig_path) ?>" id="el_com_sig" style="position:absolute; object-fit:contain; z-index: 1;">
            <?php endif; ?>
            <?php if($inspector_sig_path): ?>
                <img src="<?= getBase64Img($inspector_sig_path) ?>" id="el_ins_sig" style="position:absolute; object-fit:contain; z-index: 1;">
            <?php endif; ?>

            <div class="el flex items-center" id="el_com_rank" style="z-index: 10;"><span class="shrink-text"><?= htmlspecialchars($req['issuer_rank'] ?? '') ?></span></div>
            <div class="el flex items-center" id="el_com_pos_lbl" style="z-index: 10;">ตำแหน่ง</div>
            <div class="el flex items-center" id="el_com_pos_val" style="z-index: 10;"><span class="shrink-text"><?= htmlspecialchars($req['issuer_pos'] ?? '') ?></span></div>
