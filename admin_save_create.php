...
if (!isset($_POST['csrf_token']) || $_POST['csrf_token'] !== $_SESSION['csrf_token']) {
    echo "<!DOCTYPE html><html><head><script src='https://cdn.jsdelivr.net/npm/sweetalert2@11'></script><link href='https://fonts.googleapis.com/css2?family=Sarabun&display=swap' rel='stylesheet'><style>body{font-family:'Sarabun',sans-serif;}</style></head><body><script>
        Swal.fire({
            icon: 'error',
            title: 'Security Error',
            text: 'CSRF Token ไม่ถูกต้อง หรือตรวจสอบไม่ผ่าน',
...
// 🟢 1.1 ป้องกันการส่งข้อมูลซ้ำ (Double Submission Prevention)
// บันทึกเวลาที่ส่งล่าสุดไว้ใน session
$last_submit = $_SESSION['last_submit_time_create'] ?? 0;
$current_time = time();
$id_card_number = trim($_POST['id_card_number'] ?? '');

if (($current_time - $last_submit) < 5 && isset($_SESSION['last_id_card_create']) && $_SESSION['last_id_card_create'] === $id_card_number) {
    // ถ้ากดย้ำๆ ภายใน 5 วินาที ด้วยเลขบัตรเดิม ให้ระงับไว้
    echo "<!DOCTYPE html><html><head><script src='https://cdn.jsdelivr.net/npm/sweetalert2@11'></script><link href='https://fonts.googleapis.com/css2?family=Sarabun&display=swap' rel='stylesheet'><style>body{font-family:'Sarabun',sans-serif;}</style></head><body><script>
        Swal.fire({
            icon: 'warning',
            title: 'กำลังบันทึกข้อมูล...',
            text: 'โปรดรอสักครู่และไม่ควรคลิกปุ่มบันทึกซ้ำกันหลายครั้ง',
            confirmButtonText: 'ตกลง'
        }).then(() => { window.history.back(); });
    </script></body></html>";
    exit;
}
$_SESSION['last_submit_time_create'] = $current_time;
$_SESSION['last_id_card_create'] = $id_card_number;
...
if (empty($rank_id)) {
    echo "<!DOCTYPE html><html><head><script src='https://cdn.jsdelivr.net/npm/sweetalert2@11'></script><link href='https://fonts.googleapis.com/css2?family=Sarabun&display=swap' rel='stylesheet'><style>body{font-family:'Sarabun',sans-serif;}</style></head><body><script>
        Swal.fire({
            icon: 'error',
            title: 'ไม่สามารถระบุยศได้',
            text: 'กรุณาเลือกยศจากรายการที่ค้นหาพบ',
            confirmButtonText: 'ตกลง'
        }).then(() => { window.history.back(); });
    </script></body></html>";
    exit;
...
if (!preg_match('/^[0-9]{13}$/', $id_card_number)) {
    echo "<!DOCTYPE html><html><head><script src='https://cdn.jsdelivr.net/npm/sweetalert2@11'></script><link href='https://fonts.googleapis.com/css2?family=Sarabun&display=swap' rel='stylesheet'><style>body{font-family:'Sarabun',sans-serif;}</style></head><body><script>
        Swal.fire({
            ...
            text: 'เลขบัตรประชาชนต้องเป็นตัวเลข 13 หลักเท่านั้น',
            confirmButtonText: 'ตกลง'
        }).then(() => { window.history.back(); });
    </script></body></html>";
    exit;
...
if (empty($birth_date) || $birth_date === '0000-00-00') {
    echo "<!DOCTYPE html><html><head><script src='https://cdn.jsdelivr.net/npm/sweetalert2@11'></script><link href='https://fonts.googleapis.com/css2?family=Sarabun&display=swap' rel='stylesheet'><style>body{font-family:'Sarabun',sans-serif;}</style></head><body><script>
        Swal.fire({
            icon: 'error',
            title: 'ข้อมูลวันเกิดไม่ถูกต้อง',
            text: 'กรุณากรอกในรูปแบบ วว/ดด/ปปปป',
    ...
    <body>
        <script>
            Swal.fire({
                ...
                text: 'เลขที่บัตร: " . ($generated_card_no ?? 'ยังไม่ออกเลข') . "',
                icon: 'success',
                timer: 3000,
